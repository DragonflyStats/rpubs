...
##Clustering Algorithm - Linkage Example}
...
*  In the very first step, the two
...
*  As we can see, this happens to two
...
\end{center}
\end{figure}
*  We continue in the same fashion until one cluster is left. By following the single linkage procedure, the last steps involve the merger
...
*  A common way to visualize the cluster analysiss progress is by drawing a
...
*  An important question is how to decide on the number of
...
*  In constructing the dendrogram, SPSS rescales the distances to a range of 025; that is, the last merging step to a one-cluster solution takes place at a
