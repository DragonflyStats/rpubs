...
#### Exploring distributions}\label{exploring-distributions}
...
#### Quick scatterplot of outcome and
...
and check model assumptions with diagnostic graphs. ​
    ...
    #### Model preliminaries}\label{model-preliminaries}
...
\end{tcolorbox}
    ...
    #### Fitting the model}\label{fitting-the-model}
...
\end{tabular}
    ...
    #### Normality of residuals: q-q plot and
...
predictor values (leverage), measured by .hat, and  residuals. The
...
so influential observations for this model will be those with 
residuals.

In the following plot: * map .hat to x, .stdresid to y * map .cooksd to
size, making more influential points r. * label the points in
...
the plot, with Cook's D rising as we move away from center (r
residual) and to the right (r leverage). No point looks too
