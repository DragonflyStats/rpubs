...
*  A perfect predictor would be described as 100\% sensitive (e.g., all sick are identified as sick) and 100\% specific (e.g., all healthy are identified as healthy); however, theoretically any predictor will possess a minimum error bound known as the \textbf{\textit{Bayes error rate}}.
...
*  This trade-off can be represented graphically as a \textbf{\textit{receiver operating characteristic curve}}.
...
*  \textbf{Sensitivity} (also called the true positive rate, or the \textbf{recall} rate) measures
...
*  \textit{(Remark: We will use the terms Sensitivity and Recall interchangeably.
Sensitivity is more commonly used in a medical context, while recall is more
commonly used in data science.)}

*  \textbf{Specificity} measures the proportion of negatives which are correctly
...
*  \textit{(Remark: Not commonly used in Data Sciences, and NOT a synoym for Precision)}
...
*   A false negative error is where a test result indicates that a condition
...
*  We can evaluate error rates by means of a training sample (to construct the
discrimination rule) and a test sample.
*  An optimistic error rate is obtained by reclassifying the training data. (In
...
*  If an independent test sample is used for classifying, we arrive at the true
error rate.
*  The true error rate (or conditional error rate) of a classifier is the
